...
type CloudStateName = "placeholder" | "orb" | "torus" | "disk" | "burst";
...
// One body travels through the page and changes state per scene: a neutral
// placeholder (hero),
// distant torus (work), flat vortex disk (focus), ringed orb (about), radial
// burst on the way into contact, then back to the placeholder. Rest keyframes keep
// flare low; the high flare values sit only on the short morph spans, so
// overexposure, streaks and star flares happen at transitions and nowhere else.
const VISUAL_KEYFRAMES: VisualKeyframe[] = [
  { at: 0, state: "placeholder", x: 0.72, y: 0.5, scale: 0.5, rotateX: 0.08, rotateY: -0.04, rotateZ: 0, alpha: 1, noise: 0.006, flare: 0.05, dust: 0.7 },
  { at: 0.55, state: "placeholder", x: 0.76, y: 0.47, scale: 0.4, rotateX: 0.14, rotateY: 0.06, rotateZ: 0, alpha: 0.7, noise: 0.009, flare: 0.06, dust: 0.55 },
  { at: 1.05, state: "torus", x: 0.86, y: 0.32, scale: 0.15, rotateX: 0.52, rotateY: -0.2, rotateZ: 0.1, alpha: 0.52, noise: 0.012, flare: 0.42, dust: 0.12 },
  { at: 1.55, state: "torus", x: 0.82, y: 0.4, scale: 0.28, rotateX: 0.46, rotateY: 0.16, rotateZ: 0.04, alpha: 0.4, noise: 0.014, flare: 0.08, dust: 0.18 },
  { at: 2.1, state: "disk", x: 0.74, y: 0.56, scale: 0.66, rotateX: 0.34, rotateY: -0.12, rotateZ: -0.05, alpha: 0.4, noise: 0.016, flare: 0.38, dust: 0.6 },
  { at: 2.6, state: "disk", x: 0.7, y: 0.52, scale: 0.56, rotateX: 0.3, rotateY: 0.08, rotateZ: 0.02, alpha: 0.34, noise: 0.014, flare: 0.07, dust: 0.62 },
  { at: 3.1, state: "orb", x: 0.55, y: 0.3, scale: 0.3, rotateX: 0.3, rotateY: 0.18, rotateZ: -0.06, alpha: 0.48, noise: 0.01, flare: 0.4, dust: 0.1 },
  { at: 3.55, state: "orb", x: 0.52, y: 0.32, scale: 0.36, rotateX: 0.26, rotateY: -0.1, rotateZ: 0.03, alpha: 0.42, noise: 0.009, flare: 0.08, dust: 0.1 },
  { at: 4.15, state: "burst", x: 0.62, y: 0.46, scale: 1.05, rotateX: 0.2, rotateY: 0.06, rotateZ: 0.12, alpha: 0.46, noise: 0.02, flare: 0.62, dust: 0 },
  { at: 4.6, state: "placeholder", x: 0.7, y: 0.5, scale: 0.48, rotateX: 0.1, rotateY: -0.05, rotateZ: 0, alpha: 0.56, noise: 0.007, flare: 0.1, dust: 0.45 },
  { at: 5, state: "placeholder", x: 0.72, y: 0.52, scale: 0.44, rotateX: 0.12, rotateY: 0.04, rotateZ: 0, alpha: 0.62, noise: 0.006, flare: 0.06, dust: 0.55 },
];
...
function makeCloud(count: number): ParticleCloud {
  const names: CloudStateName[] = ["placeholder", "orb", "torus", "disk", "burst"];
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    // Neutral filler only. The future logo implementation owns this state;
    // nothing here encodes or hints at the previous mark.
    const placeholderRadius = Math.cbrt(seedA) * 0.7;
    const placeholderPole = seedB * 2 - 1;
    const placeholderAngle = seedC * TAU;
    const placeholderCross = Math.sqrt(Math.max(1 - placeholderPole * placeholderPole, 0));
    states.placeholder.x[index] = Math.cos(placeholderAngle) * placeholderCross * placeholderRadius;
    states.placeholder.y[index] = placeholderPole * placeholderRadius;
    states.placeholder.z[index] = Math.sin(placeholderAngle) * placeholderCross * placeholderRadius;
    ...
    // torus — main tube with a thin inner companion ring; a third of the tube
    // particles clump into five broad arcs so the ring reads broken and uneven
    const innerRing = index % 7 === 0;
    const major = innerRing ? 0.44 : 0.74;
    const minor = innerRing ? 0.018 : 0.07 + seedC * 0.06;
    const arcCenter = (Math.floor(seedD * 5) / 5) * TAU + 0.5;
    const around = innerRing || index % 3 !== 0
      ? seedA * TAU
      : arcCenter + (seedA - 0.5) * 1.6;
    const tube = seedB * TAU;
    const torusRadius = major + Math.cos(tube) * minor;
    states.torus.x[index] = Math.cos(around) * torusRadius;
    states.torus.z[index] = Math.sin(around) * torusRadius;
    states.torus.y[index] = Math.sin(tube) * minor * 1.5 + (seedD - 0.5) * 0.02;

    // disk — flat two-armed vortex with a dense core and a ragged outer rim
    const inCore = index % 9 === 0;
    const diskRadius = inCore ? Math.pow(seedA, 0.8) * 0.2 : 0.1 + Math.pow(seedA, 0.55) * 0.95;
    ...
    const brightness = Math.min(2, Math.floor(seeded(index, 10) * 3.1));
...
function makeDustTexture(size: number, count: number) {
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    const x = center + Math.cos(angle) * extent * radius * radialNoise;
    const y = center + Math.sin(angle) * extent * radius * radialNoise;
    const edgeFade = clamp((1 - radius) / 0.13);
...
export function CosmicUniverse() {
  ...
  useEffect(() => {
    ...
    const dustTexture = makeDustTexture(constrained ? 720 : 1024, constrained ? 9000 : 16000);
    ...
    let sceneBounds: { top: number; height: number }[] = [];

    const timelineAt = (scrollY: number) => {
      let timeline = 0;
      for (let index = 0; index < sceneBounds.length; index += 1) {
        const bounds = sceneBounds[index];
        if (scrollY < bounds.top) break;
        timeline = index + clamp((scrollY - bounds.top) / bounds.height);
      }
      return timeline;
    };

    const updateSceneBounds = () => {
      sceneBounds = Array.from(document.querySelectorAll<HTMLElement>(".scene")).map((scene) => {
    ...
    const draw = (time: number) => {
      ...
      const alphaScale = mobile ? 0.78 : 1;
      ...
      const centerY = (lerp(from.y, to.y, mix) - (mobile ? 0.04 : 0)) * height
      ...
      if (dustAmount > 0.02) {
        ...
        context.globalAlpha = clamp(alpha * dustAmount * (0.9 + flare * 0.25), 0, 0.72);
        context.drawImage(
          dustTexture,
          -dustRadius,
          -dustRadius,
      ...
      const coreGlowSize = radius * (0.18 + flare * 0.35);
      context.globalAlpha = alpha * (0.12 + flare * 0.32);
      context.drawImage(
        glowSprite,
      ...
      const hotCoreSize = radius * (0.05 + flare * 0.12);
      context.globalAlpha = clamp(alpha * (0.16 + flare * 0.5), 0, 0.72);
      context.drawImage(
        glowSprite,
      ...
      for (let bucket = 0; bucket < cloud.buckets.length; bucket += 1) {
        ...
        context.globalAlpha = clamp(bucketAlpha[bucket] * alpha * (1 + flare * 0.34), 0, 1);
        ...
        for (let listIndex = 0; listIndex < indices.length; listIndex += 1) {
          ...
          if (bucket === 3 && listIndex % 5 === 0) {
            const glowSize = 9 + particleSize * 4 + flare * 18;
            context.globalAlpha = alpha * (0.045 + flare * 0.065);
            context.drawImage(
              glowSprite,
              screenX - glowSize / 2,
              screenY - glowSize / 2,
